...
def index():
    ...
    stats = {
        ...
        'vehiculos':       scalar(db, 'SELECT COUNT(*) FROM vehiculos'),
...
def ver_cliente(id):
    db = get_db()
    cliente  = fetchone(db, 'SELECT * FROM clientes WHERE id=?', (id,))
    vehiculos = fetchall(db, 'SELECT * FROM vehiculos WHERE cliente_id=?', (id,))
    ordenes   = fetchall(db, '''
...
def nuevo_vehiculo():
    ...
    if request.method == 'POST':
        ...
        if not marca or not modelo or not patente:
            ...
            try:
                execute(db, 'INSERT INTO vehiculos (marca,modelo,patente,anio,motor,vin,color,combustible,cliente_id) VALUES (?,?,?,?,?,?,?,?,?)',
                        (marca, modelo, patente, anio, motor, vin, color, combustible, cliente_id))
                db.commit(); db.close()
                flash('Vehículo agregado.', 'success')
                return redirect(url_for('vehiculos'))
            except Exception:
                flash('La patente ya existe.', 'danger')
    db.close()
    return render_template('vehiculo_form.html', vehiculo=None, clientes=clientes_list)

@app.route('/vehiculos/<int:id>/editar', methods=['GET', 'POST'])
def editar_vehiculo(id):
    db = get_db()
    vehiculo = fetchone(db, 'SELECT * FROM vehiculos WHERE id=?', (id,))
    clientes_list = fetchall(db, 'SELECT id, nombre || \' \' || apellido AS nombre FROM clientes ORDER BY apellido')
    if request.method == 'POST':
        try:
            execute(db, 'UPDATE vehiculos SET marca=?,modelo=?,patente=?,anio=?,motor=?,vin=?,color=?,combustible=?,cliente_id=? WHERE id=?',
            ...
            flash('La patente ya existe.', 'danger')
    db.close()
    return render_template('vehiculo_form.html', vehiculo=vehiculo, clientes=clientes_list)
...
def ver_orden(id):
    ...
    fotos = fetchall(db, 'SELECT * FROM orden_fotos WHERE orden_id=? ORDER BY id', (id,))
    db.close()
    return render_template('orden_detalle.html', orden=orden, repuestos=repuestos, fotos=fotos)

@app.route('/ordenes/<int:id>/fotos', methods=['POST'])
def subir_foto_orden(id):
...
@app.route('/ordenes/<int:id>/fotos/<int:foto_id>/eliminar', methods=['POST'])
def eliminar_foto_orden(id, foto_id):
    db = get_db()
...
def nuevo_presupuesto():
    ...
    db.close()
    return render_template('presupuesto_form.html', presupuesto=None, clientes=clientes_list, vehiculos=vehiculos_list, items=[])
...
def editar_presupuesto(id):
    ...
    db.close()
    return render_template('presupuesto_form.html', presupuesto=presupuesto, clientes=clientes_list, vehiculos=vehiculos_list, items=items)
...
def exportar_ordenes():
    ...
    headers = ['N°','Ingreso','Egreso','Estado','Cliente','Teléfono','Marca','Modelo','Patente','Año','Diagnóstico','Trabajo','Repuestos']
    ...
    return send_file(output, download_name='ordenes_taller.xlsx', as_attachment=True,
...
def exportar_clientes():
    ...
    for row in rows:
        ws.append(list(row.values()))
    output = io.BytesIO(); wb.save(output); output.seek(0)
    return send_file(output, download_name='clientes_taller.xlsx', as_attachment=True,
                     mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
